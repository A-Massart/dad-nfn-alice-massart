...
async function chargerJournalEtMotsCles(file) {
  const dateStr = extraireDateDuFichier(file.name);
  
  if (!dateStr) {
    console.warn("Format de fichier non reconnu. Veuillez utiliser le format: YYYY-MM-DD_journal-de-bord.md");
  } else {
    await chargerMotsClesParDate(dateStr);
  }
  
  // Lire et afficher le contenu du journal
